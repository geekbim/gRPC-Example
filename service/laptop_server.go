...
func (server *LaptopServer) UploadImage(stream pb.UploadImageServer) error {
	req, err := stream.Recv()
	if err != nil {
	...
	log.Printf("receive an upload-iamge request for laptop %s with image type %s", laptopID, imageType)

	laptop, err := server.laptopStore.Find(laptopID)
	if err != nil {
		return logError(status.Errorf(codes.Interal, "cannot find laptop: %s", err))
	}
	if laptop == nil {
	...
	for {
		...
		if err != nil {
			return logError(status.Errof(codes.Unknown, "cannot receive chunk data: %v", err))
		}
		...
		if err != nil {
			return logError(status.Errorf(codes.Internal, "cannot writ chunk data: %v", err))
		}
	}

	imageId, err := server.imageStore.Save(laptopID, imageType, imageData)
	if err != nil {
		return log.Error(status.Errorf(codes.Internal, "cannot save image to the store: %v", err))
	}

	res := &pb.UplaodImageResponse{
		Id: imageId,
...
func (server *LaptopServer) RateLaptop(stream pb.LaptopService_RateLaptopServer) error {
	for {
		...
		if found == nil {
			return logError(status.Errorf(codes.NotFound, "laptopID %sis not found", laptopID))
		}
